...
class OptionPricingModels():
    ...
    #     return price
    def __init__(self, S, K, T, r, sigma, option_type):
        # S, K, T, r, sigma, option_type can be vectors (arrays) now
        self.S = np.array(S)
        self.K = np.array(K)
        self.T = np.array(T)
        self.r = r
    ...
    def new_monte_carlo_option_price(self, ticker=None, output_folder=None, num_simulations=10000):
        dt = 1 / 365  # Daily steps
        # Check if self.T is a single value or an array
        # Ensure num_steps is always an array
        num_steps = (self.T * 365).astype(int) if isinstance(self.T, np.ndarray) else np.array([int(self.T * 365)])
        ...
        for _ in range(num_simulations):
            ...
            for idx in range(len(S_array)):
                # Make sure num_steps is always an array
                current_steps = num_steps[idx] if len(num_steps) > 1 else num_steps[0]  # Handle single/multiple options
        ...
        return option_prices, plot_filenames if output_folder is not None else None
